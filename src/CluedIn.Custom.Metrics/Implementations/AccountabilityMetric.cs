...
{
    public class AccountabilityMetric : PercentageMetric
    {
        private readonly IMetricProviderResolver providerResolver;

        public AccountabilityMetric(IMetricProviderResolver providerResolver)
        {
        ...
        public override Guid Id { get; } = new Guid("{713ED099-049F-480F-9BF0-D6C49A192CAD}");
        ...
        {
            return edges.Where(e => !getReferencePoint(e).Type.Is(Core.Data.EntityType.Temporal)
                                 && !getReferencePoint(e).Type.Is(Core.Data.EntityType.Provider.Root));
        }

        private static double CalculateMetricValue(MetricsProcessingContext context, Entity entity)
        {
            //TODO: This needs to look up WHO is responsible all the record. 
            ...
            var uniqueSources = parts.Select(i => i.EntityData.ProviderDefinitionId).Distinct().Where(n => n != null).ToList();

           // var totalGroups = uniqueSources.Count();

           var averageTrustScore = context.Organization.DataStores.GetDataStore<OrganizationProviderOwner>()?.Select(context, n => uniqueSources.Contains(n.ProviderDefinitionId));

           // var averageTrustScore = uniqueSources.Select(n => context.Organization.DataStores.GetDataStore<OrganizationProviderOwner>()?.GetById(context, n.Value)).Where(n => n != null).ToList();

            double score = averageTrustScore.Count();
            if (!uniqueSources.Any())
                return 0f;

            return score / (double)uniqueSources.Count();
        }
