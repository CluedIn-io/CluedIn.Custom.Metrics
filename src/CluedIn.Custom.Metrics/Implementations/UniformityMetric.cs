...
{
    public class UniformityMetric : PercentageMetric
    {
        private readonly IMetricProviderResolver providerResolver;

        public UniformityMetric(IMetricProviderResolver providerResolver)
        {
        ...
        public override Guid Id { get; } = new Guid("{98DB0F76-1577-4874-8A83-6B11E11FFA09}");
            ...
            Entity entity)
        {
            ...
            if (dimension.DimensionType.HasFlag(MetricDimensionType.Global))
            {
                ...
                switch (dimension.DimensionType)
                {
                    case MetricDimensionType.GlobalIntegration:
                        ...
                        explanation = this.GetAggregatedValueExplanation(context, MetricDimensionType.EntityIntegration);
                        break;
                    ...
                    case MetricDimensionType.GlobalIntegrationType:
                        ...
                        explanation = this.GetAggregatedValueExplanation(context, MetricDimensionType.EntityIntegrationType);
                        break;
        ...
        private static double CalculateMetricValue(UniqueValuesMap v)
        {
            ...
            var average = v.MaxUniqueValueCount / (v.ValuesCount + (v.MissingValuesCount / (double)v.BranchCount));
        ...
        public override IEnumerable<IMetricDimension> GetDimensions(MetricsProcessingContext context, IMetricsModel model)
        {
            ...
            var entityIntegrationDimensions = existingDimensions.Where(
                d =>
                    d.DimensionType == MetricDimensionType.EntityIntegration
                    && d.DimensionDetailType == MetricDimensionDetailType.None
                    && d.ProviderDefinitionId != null && d.ProviderId != null);
            ...
            var entityIntegrationTypeDimensions = existingDimensions.Where(
                d =>
                    d.DimensionType == MetricDimensionType.EntityIntegrationType
                    && d.DimensionDetailType == MetricDimensionDetailType.None
                    && d.ProviderId != null);
        ...
        public override IEnumerable<IMetricDimension> GetDimensionsToCalculate(MetricsProcessingContext context, Entity entity)
        {
            ...
            // Entity Property
            {
                ...
                {
                    yield return new MetricDimension(context, this, entityLevelDimension, MetricDimensionType.Entity, MetricDimensionDetailType.Property, dimensionDetail: key);
                }
        ...
        {
            var groups = mergedBranches.Select(m => func(m.MergedData)).GroupBy(v => v == null ? v : v.ToLowerInvariant());
            var headGroups = mergedBranches.Select(m => func(m.HeadData)).GroupBy(v => v == null ? v : v.ToLowerInvariant());
