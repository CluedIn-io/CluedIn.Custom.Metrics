...
{
    public class TimelinessMetric : PercentageMetric
    {
        private readonly IMetricProviderResolver providerResolver;

        public TimelinessMetric(IMetricProviderResolver providerResolver)
        {
        ...
        public override Guid Id { get; } = new Guid("{007E1950-E686-4C12-B0EA-7B8E6234CA87}");
        ...
        {
            //Find out the different between discoverability date and the use within streams. 
            var dbContext = context.ApplicationContext.Container.Resolve<CluedInEntities>();

            double cc = dbContext.StreamIngestionLog.Count(i => i.EntityId == entity.Id && EF.Functions.DateDiffHour(i.DateSent.Value.Date, entity.DiscoveryDate.Value.DateTime) > 1);
            double cc2 = dbContext.StreamIngestionLog.Count(i => i.EntityId == entity.Id);
