...
{
    public class ReliabilityMetric : PercentageMetric
    {
        private readonly IMetricProviderResolver providerResolver;

        public ReliabilityMetric(IMetricProviderResolver providerResolver)
        {
        ...
        public override Guid Id { get; } = new Guid("{ABD97490-DBA4-4FC7-B11C-E984024DA910}");
            ...
            Entity entity)
        {
            ...
            if (dimension.DimensionType.HasFlag(MetricDimensionType.Entity))
            {
                ...
                {
                    var provider = context.Organization.Providers.GetProviderDefinition(context, dimension.ProviderDefinitionId.Value);
                    
                    if (provider == null)
                        return new PercentageMetricValue(dimension, entity.Id, 0f);
        ...
        {
            //Get the providerDefinition Trust Score
            ...
            var uniqueSources = parts.GroupBy(i => i.EntityData.ProviderDefinitionId).ToList();

            var totalGroups = uniqueSources.Count();

            try
            {
                var averageTrustScore = uniqueSources.Where(n => n.Key.HasValue).Select((a, b) => context.Organization.Providers.GetProviderDefinition(context, a.Key.Value));

                double? score = averageTrustScore.Average(d => d?.SourceQuality ?? 0f);

                return score.Value / 10f;
            }
            catch (Exception)
            {
                return 0;
