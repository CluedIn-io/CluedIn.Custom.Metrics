...
{
    public class AccountEntityMetricsTestsFixture : TestContextFixture
    {
        public AccountEntityMetricsTestsFixture()
        {
            var serializer  = new XmlSerializer(this.TestContext.Context, SerializationFlavor.Persisting);
            this.Entity = serializer.Deserialize<Entity>(File.ReadAllText("TestData/Metrics/account.entity.xml"));
        }

        public Entity Entity { get; set; }
    }

    public class AccountEntityMetricsTests : MetricsImplementationTestsBase, IClassFixture<AccountEntityMetricsTestsFixture>
    {
        private readonly AccountEntityMetricsTestsFixture fixture;

        private readonly ITestOutputHelper outputHelper;

        private Entity entity;

        public AccountEntityMetricsTests(AccountEntityMetricsTestsFixture fixture, ITestOutputHelper outputHelper)
            : base(outputHelper)
        {
        ...
        [Fact]
        public void AvailabilityMetricTest()
        {
            var (globalMetric, _) = this.TestMetric(new DarkDataMetric(this.ProviderResolver), this.entity);
