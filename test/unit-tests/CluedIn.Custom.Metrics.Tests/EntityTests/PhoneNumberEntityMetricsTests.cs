...
{
    public class PhoneNumberEntityMetricsTestsFixture : TestContextFixture
    {
        public PhoneNumberEntityMetricsTestsFixture()
        {
            var serializer  = new XmlSerializer(this.TestContext.Context, SerializationFlavor.Persisting);
            this.Entity = serializer.Deserialize<Entity>(File.ReadAllText("TestData/Metrics/phonenumber.entity.xml"));
        }

        public Entity Entity { get; set; }
    }

    public class PhoneNumberEntityMetricsTests : MetricsImplementationTestsBase, IClassFixture<PhoneNumberEntityMetricsTestsFixture>
    {
        private readonly PhoneNumberEntityMetricsTestsFixture fixture;

        private readonly ITestOutputHelper outputHelper;

        private Entity entity;

        public PhoneNumberEntityMetricsTests(PhoneNumberEntityMetricsTestsFixture fixture, ITestOutputHelper outputHelper)
            : base(outputHelper)
        {
        ...
        public override void InterpretabilityMetricTest()
        {
            ...
            Assert.Equal(0.0136f, ((Percentage)globalMetric).ToFloat());
        }
